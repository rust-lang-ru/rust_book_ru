...
Наша строка кода заканчивается точкой с запятой (`;`). Rust — [язык с ориентацией на
выражения][expression-oriented language], а это означает, что в нём большая
часть вещей является выражением. Символ `;` используется для указания конца выражения
и начала следующего. Большинство строк кода на Rust заканчивается символом `;`.
